...
const translations: Record<Locale, Record<string, string>> = {
  en: {
    ...
    'capability.info.unweightedDesc': 'The textbook form assuming unit joint-torque norm. Shows the pure directional shape of force capability. No torque limit data needed, but the absolute size is not in physical units.',
    'capability.info.rmsTitle': 'RMS-scaled',
    'capability.info.rmsDesc': 'Scales the unweighted form by the RMS of joint torque limits to give physically meaningful size. The shape is identical — only the scale changes.',
    'capability.info.weightedTitle': 'Torque-weighted',
    'capability.info.weightedDesc': 'Scales each Jacobian column by the joint\'s actual torque limit, changing both shape and scale. Produces the tightest ellipsoidal fit to the polytope. Most visible on robots with asymmetric torque limits. When all joints have equal limits, this reduces to the RMS-scaled version.',
  ...
  ja: {
    ...
    'capability.info.unweightedDesc': '単位関節トルクノルムを仮定した教科書的な形式です。力能力の純粋な方向的形状を示します。トルク制限データ不要ですが、絶対的なサイズは物理単位ではありません。',
    'capability.info.rmsTitle': 'RMSスケーリング',
    'capability.info.rmsDesc': '非重み付き形式を関節トルク制限のRMSでスケーリングし、物理的に意味のあるサイズにします。形状は同一で、スケールのみが変化します。',
    'capability.info.weightedTitle': 'トルク重み付き',
    'capability.info.weightedDesc': '各ヤコビアン列を関節の実際のトルク制限でスケーリングし、形状とスケールの両方を変更します。ポリトープに最も近い楕円体近似を実現します。非対称なトルク制限を持つロボットで最も顕著です。全関節の制限が等しい場合、RMSスケーリング版と同一になります。',
  ...
  zh: {
    ...
    'capability.info.unweightedDesc': '假设单位关节扭矩范数的教科书形式。显示力能力的纯方向形状。不需要扭矩限制数据，但绝对大小不是物理单位。',
    'capability.info.rmsTitle': 'RMS缩放',
    'capability.info.rmsDesc': '用关节扭矩限制的RMS缩放非加权形式，给出物理意义的大小。形状相同，仅尺度变化。',
    'capability.info.weightedTitle': '扭矩加权',
    'capability.info.weightedDesc': '按每个关节的实际扭矩限制缩放雅可比矩阵列，同时改变形状和尺度。产生与多面体最紧密的椭球拟合。在扭矩限制不对称的机器人上最为明显。当所有关节限制相等时，退化为RMS缩放版本。',
