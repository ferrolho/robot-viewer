...
**Model hosting:** Robot models are processed and hosted in a separate repo ([ferrolho/robot-explorer-models](https://github.com/ferrolho/robot-explorer-models)) and served via `raw.githubusercontent.com` from the `dist` branch. The base URL can be overridden with the `VITE_MODELS_BASE_URL` env var.
